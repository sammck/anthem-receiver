...
CommandCode = bytes
"""A command code. A byte string like b'ZxTBS?' or b'z<zone:int:1>TBS?' that defines a particular command.
   Lowercase letters are used for variable numeric bytes (1-n chars, where n is
   the number of repetitions of the letter) in the command. "<name:type:length>"
   or <name:type:min_length:max_length> is used to define a variable field of any type. """

ResponseCode = bytes
"""A response code. A byte string like b'SIPx' or b'IDQ<model_version:str:1:32>'that defines a particular response.
   Lowercase letters are used for numeric bytes in the final response. "<name:type:length>"
   or <name:type:min_length:max_length> is used to define a variable field of any type."""

class PacketPatternPart:
    part_type : type
    """The type of this part of the packet pattern. One of PacketType, CommandCode, ResponseCode, or bytes."""
...
class PacketPattern:
    ...
    def __str__(self) -> str:
        return f"PacketPattern({self.pattern_str})"

    def __repr__(self) -> str:
        return str(self)
class CommandMeta:
    command_code: CommandCode
    """The command code for this command. A byte string like b`ZxTBS?` that defines a particular command.
